...
def test_api_simulation_response_exposes_source_block_at_e(monkeypatch, tmp_path):
    ...
    assert [stage.stage for stage in result.diagnostics.stageDurations] == ["A_B", "B_C", "C_D", "D_E"]
...
def test_frozen_frontend_fixture_is_identified_as_pre_milestone16_reference():
    ...
    note = Path("docs/api/examples/README.md").read_text(encoding="utf-8")
    ...
    assert data["physicalScope"].startswith("A_TO_F certified:")
    assert "Milestone 1.5" in note
    assert "NOT_SOURCE_CERTIFIED_A_TO_F" in note
