...
def test_figure_time_is_absolute_and_event_error_is_not_origin_shift(chain):
 rows=event_table(*chain[1:]);assert rows[-1].absolute_s==pytest.approx(481.4794399,rel=2e-7);assert rows[-1].error_s>180;assert rows[-1].duration_s==pytest.approx(422.0498545,rel=2e-7)

def test_quantitative_rmse_exposes_velocity_and_position_discrepancy(chain):
 r=trajectory_rmse(*chain);assert r['pc1']>2.9 and r['h_l']>300 and r['v_l']>1.8
