...
from .valves import motor_valve_gas_rate, santos_glv_mass_rate
...
def common_to_stage_bc_result(
    result: StageBCCommonResult, params: GLIParameters
) -> StageBCResult:
    ...
    for i in range(n):
        ...
        pc1[i] = casing["p_c1"]
        pc2[i] = casing["p_c2"]
        mgl[i] = santos_glv_mass_rate(casing["p_c2"], s[I_PG, i], params)
        qm[i] = motor_valve_gas_rate(
            casing["p_c1"],
            params.operating.injection_pressure_pa,
            params.fluids.gas_relative_density,
            params.gas.temp_c1_k,
    ...
    return StageBCResult(
        ...
        mgl,
        s[I_VGI],
        result.target_volume_std_m3,
        result.event_c_reached,
        result.event_c_time_s,
...
def initial_state_b(
    ...
    ab: Stage1Result,
    seed_height_m=1e-3,
    seed_film_fraction=0.0199,
    initial_vf_m_s=0.0,
    enforce_santos_slug_constraint=False,
):
    ...
    y = r * (1 - sqrt(1 - seed_film_fraction))
    Ab = pi * (r - y) ** 2
    Af = At - Ab
    V = Ab * seed_height_m
    mg = (
        ...
        * V
        / (g.z_t1 * g.gas_constant_j_mol_k * g.temp_t1_k)
    )
    rho = mg / V
    mc = float(ab.annulus_gas_mass_kg[-1] - mg)
    hl = params.geometry.initial_slug_length_m + (Ab / At) * seed_height_m
    vl = 0.0152
    vb = params.coefficients.bubble_velocity_a * vl + 0.35 * sqrt(
        GRAVITY_M_S2 * params.geometry.tubing_diameter_m
    )
    if enforce_santos_slug_constraint:
        initial_vf_m_s = (At * vl - Ab * vb) / max(Af, 1e-18)
    mf = ini["rho_l"] * Af * seed_height_m
    rho_std = standard_gas_density(params)
    vgi = float((ab.annulus_gas_mass_kg[-1] - ab.annulus_gas_mass_kg[0]) / rho_std)
    return np.array(
        [
            mc,
            mg,
...
def frozen_bubble_friction_factor(params: GLIParameters, state: np.ndarray) -> float:
    """Fixed f_B required by differentiating Santos 4.1.27 into 4.1.28."""
    from .stage_ef_dynamic import default_stage_ef_parameters

    rho, vg, y = state[I_RHO], state[I_VG], state[I_Y]
    diameter = params.geometry.tubing_diameter_m
    closure = FrictionClosure(default_stage_ef_parameters().roughness)
    return friction_factor(
        max(rho * abs(vg) * diameter / 1.1e-5, 1e-12), diameter, closure
    )[0]
...
def rhs_bc_common(_t, s, params: GLIParameters):
    ...
    rho_std = standard_gas_density(params)
    mgl = santos_glv_mass_rate(casing["p_c2"], pg, params)
    qm = motor_valve_gas_rate(
        casing["p_c1"],
        params.operating.injection_pressure_pa,
        params.fluids.gas_relative_density,
        g.temp_c1_k,
    ...
    ff, _ = friction_factor(
        max(rho_l * abs(vl) * D / params.fluids.liquid_viscosity_pa_s, 1e-9), D, fc
    )
    dvl = (
        pg
        - ini["p_t3"]
        - rho_l * GRAVITY_M_S2 * slug
        - ff * 0.5 * rho_l * vl * abs(vl) * slug / D
    ) / (rho_l * slug)
    ...
    fg, _ = friction_factor(
        max(rho * abs(vg) * 2 * (r - y) / 1.1e-5, 1e-9), max(2 * (r - y), 1e-9), fc
    )
    tau = fg * rho * vg * vg / 8
    ...
    dprod = 0.0
    return np.array(
        [dmc, dmg, drho, dpg, dvg, dvf, dy, dmfilm, dhb, dhl, dvl, qm, dfb, dprod]
    )


def rhs_bc_santos_compatible(
    _t, s, params: GLIParameters, *, bubble_friction_factor=None
):
    ...
    pt2 = pg - rho * GRAVITY_M_S2 * hb - fg * 0.5 * rho * vg * abs(vg) * hb / D
    H = params.geometry.valve_depth_m
    pt3 = params.operating.surface_tubing_pressure_pa * exp(
        g.gas_molar_mass_kg_mol
        * GRAVITY_M_S2
        * max(H - hl, 0)
    ...
    return np.array(
        [dmc, dmg, drho, dpg, dvg, dvf, dy, dmfilm, dhb, dhl, dvl, qm, 0.0, 0.0]
    )


def simulate_stage_b_to_c_common(
    params: GLIParameters,
    *,
    stage_a_b=None,
    max_time_s=180,
    ...
    s0 = initial_state_b(
        params,
        ab,
        seed_height_m=seed_height_m,
        seed_film_fraction=seed_film_fraction,
        initial_vf_m_s=initial_vf_m_s,
        enforce_santos_slug_constraint=compatible,
    )
    target = injected_gas_target_std_m3(params)
    bubble_friction = frozen_bubble_friction_factor(params, s0)
    rhs = rhs_bc_santos_compatible if compatible else rhs_bc_common

    def fun(t, s):
        if compatible:
            return rhs(t, s, params, bubble_friction_factor=bubble_friction)
        return rhs(t, s, params)

    def ev(t, s):
        return s[I_VGI] - target
    ...
    rho_std = standard_gas_density(params)
    gas = s[I_MC] + s[I_MG]
    expected = gas[0] + rho_std * (s[I_VGI] - s[I_VGI, 0])
    ge = float(np.max(abs(gas - expected)) / max(abs(gas[-1] - gas[0]), 1))
    g = params.gas
    D = params.geometry.tubing_diameter_m
    r = D / 2
    Ab = pi * (r - s[I_Y]) ** 2
    ...
    peos = (
        ...
        * g.temp_t1_k
        / (g.gas_molar_mass_kg_mol * np.maximum(V, 1e-12))
    )
    ee = float(np.max(abs(peos - s[I_PG]) / np.maximum(peos, 1)))
    At = tubing_area(D)
    liq = (
        At * (s[I_HL] - s[I_HB])
        + s[I_MFILM] / initial_stage_1(params)["rho_l"]
        + s[I_FB]
        + s[I_PROD]
    )
    ...
    native_reservoir = np.concatenate(
        ([0.0], np.cumsum(0.5 * (native_q[1:] + native_q[:-1]) * np.diff(sol.t)))
    )
    ...
    physical = bool(
        np.all(s[I_RHO] > 0)
        and np.all(s[I_PG] > 0)
        and np.all((s[I_Y] > 0) & (s[I_Y] < r))
        and np.max(abs(s[I_VG])) < 20
        and np.max(abs(s[I_VF])) < 20
    )
    inflow_valid = bool(
        all(
            reservoir_inflow_from_pt1(
                params, float(x), initial_stage_1(params)["rho_l"]
            ).physically_valid
            for x in s[I_PG]
        )
    )
