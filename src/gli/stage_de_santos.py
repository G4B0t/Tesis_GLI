...
def stage3_factors(params, rho, vb, vl, *, frozen_bubble_friction: float | None = None):
    ...
    fg = (
        frozen_bubble_friction
        if frozen_bubble_friction is not None
        else friction_factor(max(rho * abs(vb) * D / 1.1e-5, 1e-12), D, fc)[0]
    )
    fl = friction_factor(
        max(rl * abs(vl) * D / params.fluids.liquid_viscosity_pa_s, 1e-12), D, fc
    )[0]
    return fg, fl
...
def stage3_initial_state(params, cd):
    if cd.canonical_states is None:
        raise ValueError(
            "Stage 3 requires the canonical Stage-2 state; no D reconstruction permitted"
        )
    state = np.zeros(21)
    ...
    fg, _ = stage3_factors(
        ...
        state[VB],
        state[VL],
        frozen_bubble_friction=cd.bubble_friction_factor,
    )
    # Algebraic Pt2 as evaluated immediately before D, no state projection.
    state[PT2] = state[PT1] - state[RHO] * state[HB] * (
        G + fg * state[VB] ** 2 / (2 * params.geometry.tubing_diameter_m)
    )
    state[PC1 : RHOC2 + 1] = [
        cd.p_c1_pa[-1],
        cd.p_c2_pa[-1],
...
def stage3_derivatives(params, s, glv_open=True, *, bubble_friction_factor=None):
    ...
    fg, fl = stage3_factors(
        params, rho, vb, vl, frozen_bubble_friction=bubble_friction_factor
    )
    force = gas_lift_valve_resultant_force(
        s[PC2], ini["p_bt"], s[PT1], params.valves.rv, params.valves.bellows_area_m2
    )
    mgv = santos_glv_mass_rate(s[PC2], s[PT1], params) if glv_open else 0.0
    ...
    d[RHOC1] = (
        ...
        * gas.gas_molar_mass_kg_mol
        / (gas.z_c1 * gas.gas_constant_j_mol_k * gas.temp_c1_k)
    )
    d[RHOC2] = (
        d[PC2]
        * gas.gas_molar_mass_kg_mol
        / (gas.z_c2 * gas.gas_constant_j_mol_k * gas.temp_c2_k)
    )
    ...
    d[PT2] = (
        d[PT1]
        - (fg * vb**2 * hb / (2 * D) + hb * G) * d[RHO]
        - fg * rho * vb * hb * d[VB] / D
        - (fg * rho * vb**2 / (2 * D) + rho * G) * vb
    )
    ...
    return d, dict(
        ...
        mgv=mgv,
        qres=qr.rate_m3_s,
        inflow_valid=qr.physically_valid,
        fg=fg,
        fl=fl,
        source_mgv=mgv,
    )
...
def simulate_stage3(
    params, cd, *, max_time_s=180.0, max_step_s=0.1, rtol=1e-8, atol=1e-10
):
    ...
    def close(_t, s):
        return stage3_derivatives(
            params, s, True, bubble_friction_factor=bubble_friction_factor
        )[1]["force"]
    ...
    if not initially_open:
        ...
        t, s, closed, tc = np.array([0.0]), s0[:, None], True, 0.0
        message = "GLV_ALREADY_CLOSED_BEFORE_STAGE3"
    else:
        sol = solve_ivp(
            lambda t, s: stage3_derivatives(
                params, s, True, bubble_friction_factor=bubble_friction_factor
            )[0],
            (0.0, max_time_s),
            s0,
        ...
        message = (
            ...
            if closed
                ...
                "E_LIMIT_NOT_LOCALIZED"
                if sol.t_events[1].size
                else "INTEGRATION_HORIZON" if sol.success else "NUMERICAL_FAILURE"
            )
        )
    ...
    terms = [
        stage3_derivatives(
            ...
            s[:, i],
            bool(opened[i]),
            bubble_friction_factor=bubble_friction_factor,
        )[1]
    ...
    eos_error = float(
        np.max(
            abs(
                ...
                - s[RHO]
                * params.gas.z_t1
                * params.gas.gas_constant_j_mol_k
                * params.gas.temp_t1_k
    ...
    diagnostics = dict(
        ...
        remaining_slug_length_m=float(H - s[HB, -1]),
        gas_inventory_relative_error=inventory_error,
        eos_relative_error=eos_error,
        ...
        source_glv_rate_at_d=float(santos_glv_mass_rate(s0[PC2], s0[PT1], params)),
        glv_open_at_d=initially_open,
        motor_valve_open=False,
        equation_numbers=list(EQUATIONS),
        closure_state=s[:, -1].tolist() if closed else None,
    ...
    return StageDEResult(
        ...
        s[VF],
        s[RHO],
        bool(np.all(array("inflow_valid"))),
        s[VRES],
        canonical_states=s,
        film_thickness_m=s[Y],
        glv_closure_time_s=tc,
        integration_end_time_s=float(t[-1]),
        terminal_reason=message,
